...
dates_url <- function(start_date, end_date, date_format) {
  ...
  return(paste0(
    "&fechapro1=", str_replace_all(format(date_df$Start, "%d-%m-%Y"), "-", "%2F"),
...
request_norm_dates <- function(type, dates) {
  ...
  data <- sapply(dates, function(x) {
    month <- str_extract_all(x, "(?<=fechapro2=)[0-9%F]+") %>%
      str_replace_all("%2F", "-") %>%
      as.Date("%d-%m-%Y")
    ...
    check1 <- html_nodes(request_html, ".contenido a") %>%
      html_text() %>%
      trimws() %>%
      toString()
    ...
    if (check1 == refresh_msg) {
      html_nodes(request_html, ".contenido a") %>%
        html_attr("href") %>%
        {
    ...
    tryCatch({
      ...
      print(paste0(
        format(month, "%m-%Y"), ": ", number_docs_effective, " ",
        names(type_num), " found"
      ))
      ...
      norm_update <- str_extract_all(raw_table[, 2], "^(\\(.+?\\))") %>%
        unlist() %>%
        trimws()
      norm_link <- html_nodes(request_html, "a.visitado") %>%
        html_attr("href") %>%
        paste0(impo_url, .)
  ...
  return(cbind.data.frame(unlist(data[1, ]), unlist(data[2, ]), unlist(data[3, ]),
    unlist(data[4, ]), str_to_title(names(type_num)),
    stringsAsFactors = F
  ) %>%
...
retry_request <- function(df, type, missing) {
  ...
  if (sum(is.na(complete)) == 0) {
    ...
    print("Failed to request every missing date, try again")
  }
}
